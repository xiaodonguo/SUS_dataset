...
from proposed.attention_module import FeatureFusionModule as FFM
import torch
...
class Model(nn.Module):
    def __init__(self, in_chans=3, num_heads=[1, 2, 4, 8], num_classes=9, depths=[3, 3, 9, 3], norm_layer=nn.BatchNorm2d, name=None, **kwargs):
        ...
        self.transformer = FFM(dims[-1], num_heads=num_heads[-1])
    ...
    def forward(self, rgb, thermal=None):
        ...
        for i in range(self.num_stages):
            ...
            if i == 3:
                out = self.transformer(enc_rgb[i], enc_thermal[i])
