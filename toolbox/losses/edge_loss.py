...
class EdgeHoldLoss(nn.Module):
    ...
    def torchLaplace(self,x):
        edge = F.conv2d(x.float(),self.laplace.cuda(),padding=1)
...
if __name__ == "__main__":
    import cv2
    img = cv2.imread('/home/noone/桌面/对比模型/HAFNet/133_2/predicts_133withglass/0.png',0)
    logits = np.array([img])
    logits = (torch.Tensor(logits).unsqueeze(0) / 255.0 < 0.5).float()
    img = cv2.imread('/home/noone/桌面/model_others/RGB_T-GLASS/test_withglass/Boundary/0.png',0)
    labels = np.array([img])
    ...
    print('EdgeHoldLoss:',EdgeHoldLoss()(logits,labels).item())