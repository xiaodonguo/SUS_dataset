...
class SUS(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        depth = Image.open(os.path.join(self.root, 'seperated_images', image_path + '_th.jpg')).convert('RGB')
        ...
        # person = Image.open(os.path.join(self.root, 'person', image_path + '.png'))
        ...
        sample['depth'] = self.ir_to_tensor(sample['depth'])
        ...
        # sample['person'] = torch.from_numpy(np.asarray(sample['person'], dtype=np.int64) / 255.).long()
...
if __name__ == '__main__':
    ...
    path = '/root/autodl-tmp/4090map/configs/SUS_RGBT.json'
    with open(path, 'r') as fp:
        cfg = json.load(fp)
    cfg['root'] = "/root/autodl-tmp/SUS_dataset/"
