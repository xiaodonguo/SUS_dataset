...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=3, resolution=(224, 224), heads=4):
        super(ResNet, self).__init__()
        self.in_planes = 64
        self.resolution = list(resolution)
        self.conv1 = nn.Conv2d(3, 64, kernel_size=7, stride=2, padding=3, bias=False)
        # self.model0 = nn.Conv2d(3, 64, kernel_size=3, stride=1, padding=1, bias=False)
...
def ResNet50(num_classes=3, resolution=(224, 224), heads=4):
...
if __name__ == '__main__':
    # batch_size 通道 图片尺寸 224×224
    # 改动，三个图片输入的都是3通道 现改成1通道
